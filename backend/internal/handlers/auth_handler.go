...
func NewAuthHandler(auth *services.AuthService, metrics *observability.Metrics) *AuthHandler {
	return &AuthHandler{auth: auth, metrics: metrics}
}
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.observe("login", "error")
		switch {
		case errors.Is(err, services.ErrUnauthorized):
			utils.WriteError(w, http.StatusUnauthorized, "Неверный email или пароль")
...
func (h *AuthHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, services.ErrConflict):
			utils.WriteError(w, http.StatusConflict, "Пользователь с таким email уже существует")
...
func (h *AuthHandler) ForgotPassword(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.auth.ForgotPassword(r.Context(), req.Email); err != nil {
		h.observe("forgot_password", "error")
		h.writeServiceError(w, err)
		return
	}

	h.observe("forgot_password", "success")
...
func (h *AuthHandler) ResetPassword(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.auth.ResetPassword(r.Context(), req.Email, req.Code, req.NewPassword); err != nil {
		h.observe("reset_password", "error")
		h.writeServiceError(w, err)
		return
	}

	h.observe("reset_password", "success")
	utils.WriteJSON(w, http.StatusOK, map[string]string{
...
func (h *AuthHandler) observe(operation, result string) {
	if h.metrics == nil {
		return
	}
	h.metrics.AuthOperationsTotal.WithLabelValues(operation, result).Inc()
}
