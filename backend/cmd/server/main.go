...
func main() {
	...
	db, err := database.New(ctx, cfg.DatabaseDriver, cfg.DatabaseURL)
	if err != nil {
	...
	if err := database.SeedReferenceData(ctx, db); err != nil {
		logger.Error("failed to seed reference data", "error", err)
		os.Exit(1)
	}
	...
	authHandler := handlers.NewAuthHandler(authService, metrics)
	userHandler := handlers.NewUserHandler(userService)
	seatHandler := handlers.NewSeatHandler(seatService, metrics)
	...
	go func() {
		logger.Info("server starting", "port", cfg.Port, "database_driver", cfg.DatabaseDriver, "database_url", cfg.DatabaseURL)
