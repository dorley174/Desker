...
const linkClass = ({ isActive }: { isActive: boolean }) =>
  isActive ? "text-foreground font-medium" : "text-muted-foreground transition hover:text-foreground";
    ...
    <header className="sticky top-0 z-50 border-b bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/70">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4">
        <div className="flex items-center gap-6">
          <Link to="/" className="text-xl font-bold tracking-tight text-primary">Desker</Link>
          {user ? (
            <nav className="hidden items-center gap-4 text-sm md:flex">
              <NavLink to="/booking" className={linkClass}>Бронирование</NavLink>
              <NavLink to="/history" className={linkClass}>Мои брони</NavLink>
          ...
          <div className="flex items-center gap-3">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem onClick={() => navigate("/booking")}><CalendarRange className="mr-2 h-4 w-4" /> Бронирование</DropdownMenuItem>
                <DropdownMenuItem onClick={() => navigate("/history")}><History className="mr-2 h-4 w-4" /> Мои брони</DropdownMenuItem>
                {isAdmin ? <DropdownMenuItem onClick={() => navigate("/admin")}><LayoutDashboard className="mr-2 h-4 w-4" /> Админ-панель</DropdownMenuItem> : null}
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => navigate("/profile")}><User className="mr-2 h-4 w-4" /> Профиль</DropdownMenuItem>
