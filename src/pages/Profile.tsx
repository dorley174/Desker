...
import { getUserDisplayName, getUserFirstName, getUserInitials, getUserLastName } from "@/lib/user-display";
...
const Profile = () => {
  ...
  if (loading) return <div className="mx-auto max-w-4xl px-4 py-8 text-muted-foreground">Загрузка профиля...</div>;
  if (!user) return null;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <Button variant="outline" asChild><Link to="/settings">Редактировать</Link></Button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <Card>
          <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
            ...
            <div><div className="text-2xl font-bold">{displayName}</div><div className="mt-2 flex justify-center gap-2"><Badge variant={isAdmin ? "default" : "secondary"}>{isAdmin ? "Администратор" : "Сотрудник"}</Badge></div></div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border p-4"><div className="flex items-center gap-2 text-sm text-muted-foreground"><UserCircle2 className="h-4 w-4" /> Имя</div><div className="mt-2 font-medium">{getUserFirstName(user)}</div></div>
            <div className="rounded-xl border p-4"><div className="flex items-center gap-2 text-sm text-muted-foreground"><UserCircle2 className="h-4 w-4" /> Фамилия</div><div className="mt-2 font-medium">{getUserLastName(user)}</div></div>
      ...
      <Card className="mt-6">
        <CardHeader><CardTitle>Активные бронирования</CardTitle></CardHeader>
